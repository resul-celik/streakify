...
const Streakifiy = ({image}) => {

  const canvasRef = useRef();
  const previewRef = useRef();
  const originalRef = useRef();
  const finalRef = useRef();
  const maxHeight = (window.innerHeight - 75) / 2;
  ...
  const [loading,setLoading] = useState(false)
    ...
    useEffect(() => {
      ...
      function streak (e) {
        ...
        console.log(parentOffsetX)
        
        /*ev.target.onmousemove = function(e) {
          var mouseX = e.clientX;
          var mouseY = e.clientY;
          var parentOffsetX = ev.target.offsetLeft
          
          line.style.left = `${parentOffsetX - mouseX}px`
        } */
      }
